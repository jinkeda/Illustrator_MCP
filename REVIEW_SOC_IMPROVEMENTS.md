# Code Review Report: MCP SOC Improvements

## Review History
- **Initial Review**: 2026-02-07
- **Re-Review**: 2026-02-07 (after fixes applied)
...
The changes add meaningful infrastructure — request logging, chunking, snapshot/restore, streaming, schema codegen, and `assert_style`. The code is well-structured and follows existing patterns.
...
## Part 2: Re-Review (Post-Fix)

### Fixes Verified

| Fix | Status | Verdict |
|-----|--------|---------|
| ID index invalidation on deletes | `ops_element.jsx:275-276` calls `invalidateIdIndex()` after successful deletes | **Fixed** |
| Streaming race condition | `push_update` and `complete_streaming` now do lookup + put entirely inside `with self._lock` (lines 127-135, 150-160). Queue bounded to `maxsize=1000`. | **Fixed** |
| Schema unification | `op_schemas.jsx` is now auto-generated (has `AUTO-GENERATED` header, timestamp `2026-02-07`). Schemas match `gen_schemas.py` exactly. | **Fixed** |
| Chunking integration | `proxy_client.py` imports chunking utils (line 29-35) and exposes `execute_op_batch_chunked()` (lines 272-351) | **Fixed** |
| CMYK support in assert_style | New `cmykMatches()` + `checkColor()` helper handles RGBColor, CMYKColor, and reports unsupported types | **Fixed** |

Additionally, the snapshot rollback in `ops_core.jsx` now deletes newly created items before restoring state (lines 466-486), fixing the previous gap where creates weren't rolled back.

### Remaining Issues

#### Medium

**1. `gen_schemas.py` default output path still targets `op_schemas_generated.jsx`** (line 370)

The manifest loads `op_schemas.jsx`, and that file is now correctly auto-generated. But the codegen default output path is still:
```python
default=Path(__file__).parent.parent / "illustrator_mcp" / "resources" / "scripts" / "op_schemas_generated.jsx"
```
Running `python -m scripts.gen_schemas` without `-o` writes to the wrong file. Should be `op_schemas.jsx` to match the manifest.

**2. `op_schemas.jsx` manifest dependency on `task_executor` is now wrong** (manifest.json:128-130)

The generated `op_schemas.jsx` no longer calls `makeError()` or uses `ErrorCodes` — it uses inline error objects (`{code: "V003", ...}`). The `task_executor` dependency in the manifest is dead weight. Not a runtime error, but misleading and causes unnecessary script loading.

**3. `execute_op_batch_chunked` timeout logic has confusing precedence** (proxy_client.py:295)

```python
timeout = timeout or config.timeout if hasattr(config, 'timeout') else 60.0
```
Python parses this as `timeout or (config.timeout if hasattr(config, 'timeout') else 60.0)`. Since `ChunkConfig` has no `timeout` attribute, the `config.timeout` path is dead code — it always falls back to `60.0`. Works correctly but is misleading. Cleaner:
```python
timeout = timeout or 60.0
```

**4. Generated `getValueType` uses `instanceof Array`** (op_schemas.jsx:413)

The codegen template uses `val instanceof Array`, while the hand-written version used `Object.prototype.toString.call(value) === "[object Array]"`. In ExtendScript (ES3), `instanceof Array` can fail across frame/context boundaries. Minor regression for edge cases — unlikely to hit in practice since all ops run in the same context.

#### Low

**5. `assert_exists` still does O(n) scan** (ops_measure.jsx:100-123)

`assert_exists` manually walks all layers instead of using the global ID index (`resolveById` / `buildIDIndex`). After the index improvements, this could be a one-liner.

**6. No `__init__.py` in `scripts/`**

`python -m scripts.gen_schemas` still requires the `scripts` directory to be a proper package. Works when run from the project root with the right PYTHONPATH, but will confuse CI or other entry points.

### Re-Review Summary

All 5 targeted fixes are correctly implemented. The codebase is in much better shape — the schema single-source-of-truth eliminates the biggest risk from the prior review, and the streaming race condition is properly resolved.

The remaining issues are minor housekeeping — updating the codegen default path, removing the stale manifest dependency, and cleaning up a dead code path. None are blocking.
