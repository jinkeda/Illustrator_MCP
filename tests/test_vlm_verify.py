...
# Apply filter
kept, filtered_count = _filter_items(raw_items, artboard)
print(f"Input: {len(raw_items)} items")
...
for it in raw_items:
    if it["name"] not in kept_names:
        b = it["bounds"]
        w = abs(b[2]-b[0]); h = abs(b[1]-b[3])
        print(f"  FILTERED: {it['name']:25s} ({it['type']:12s}) {w:.0f}x{h:.0f}pt")

# Composite
with open(r"C:\Users\k.jin\AppData\Local\Temp\vlm_stress_base.png", "rb") as f:
    raw_png = f.read()
dims = get_png_dimensions(raw_png)

annotations = []
for i, item in enumerate(kept):
    bounds_px = map_bounds_to_pixels(item["bounds"], artboard, dims)
    annotations.append({"label": str(i+1), "bounds_px": bounds_px})

annotated = composite_overlay(raw_png, annotations)
out = r"C:\Users\k.jin\AppData\Local\Temp\vlm_refined_annotated.png"
with open(out, "wb") as f:
    f.write(annotated)
print(f"\nRefined overlay: {len(annotated)} bytes -> {out}")
